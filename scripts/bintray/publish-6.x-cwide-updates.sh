...
RELEASE=5.1.2
ARCHIVE_NAME=ilg.gnumcueclipse.repository-${RELEASE}-202009201439.zip

(
  curl -L -v https://download.eclipse.org/embed-cdt/releases/5.1.2/${ARCHIVE_NAME} -o ilg.gnumcueclipse.repository-${RELEASE}-SNAPSHOT.zip

  mkdir -p repository
  cd repository
  rm -rfv *

  unzip ../ilg.gnumcueclipse.repository-${RELEASE}-SNAPSHOT.zip
)
...
FULL_VERSION=${RELEASE}-202009201439 # "$(grep "unit id='ilg.gnumcueclipse.core'" targetPlatformRepository/content.xml | sed "s/.*version='\(.*\)'.*/\1/")"
